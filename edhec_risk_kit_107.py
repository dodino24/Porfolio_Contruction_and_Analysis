...
def plot_ef(n_points, er, cov,show_cml = False,style = '.-', rf = 0,show_ew = False,show_gmv = False):
    ...
    if show_gmv:
        ...
        ax.plot([vol_gmv],[r_gmv], color = 'blue', marker ='o', linestyle = 'dashed', markersize =10, linewidth = 2) 
    if show_ew:
        n = er.shape[0]
        ...
        ax.plot([vol_ew], [r_ew], color = 'yellow', marker = 'o', linestyle = 'dashed', markersize = 10, linewidth = 2)
    if show_cml:
        ax.set_xlim(left=0)
        wmsr = msr(rf,er,cov)
        rmsr = portfolio_return(wmsr,er)
        vmsr = portfolio_vol(wmsr,cov)
        cml_x = [0, vmsr]
        cml_y = [rf, rmsr]
...
def msr(rf, er, cov):
    """
    risk_free_rate + expected return + covariance -> weights.
    """
    n = er.shape[0]
    init_guess = np.repeat(1/n, n)
    bounds = ((0.0, 1.0),) * n # an N-tuple of 2-tuples!
    ...
    def neg_sharpe_ratio(w, rf, er, cov):
        '''
        Returns the negative of the sharpie ratio, given weights.
        '''
